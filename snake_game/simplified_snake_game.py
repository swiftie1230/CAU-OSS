...
pygame.init()
pygame.mixer.init()
pygame.mixer.music.load("snake_game/sounds/background_music.mp3")
...
def gameover_dual(screen, player):
    ...
    while running:
        ...
        # show GAME OVER !!!
        titletext = gameDisplay.blit(
            gameOverImg, (400 - (gameOverImg.get_width()/2), 300))
        ...
        # show Go menu Button
        menuButton = Button(goMenuImg, 400 - (goMenuImg.get_width()/2), 510, goMenuImg.get_width(),
                            goMenuImg.get_height(), clickgoMenuImg, (400 - clickgoMenuImg.get_width()/2), 508, mainmenu)
...
def showrank():
    ...
    while menu:
        ...
        if(is_it_rank()):
            quitButton = Button(quitImg, 600, 400-Button_height/2, Button_width,
                                Button_height, clickQuitImg, 598, 400-Button_height/2, mainmenu)
            tmp = 0
            for name, sc in ranking.items():
                rank = myfont.render(str(tmp+1) + ". " +
                                     name + " : " + str(sc), 1, (0, 0, 0))
                # SCREEN -> 800 120~570
            ...
            quitButton = Button(quitImg, 400-Button_width/2, 600, Button_width,
                                Button_height, clickQuitImg, 400-Button_width/2, 600, mainmenu)
...
def pausemenu():
    ...
    while menu:
        ...
        pausetitletext = gameDisplay.blit(
            pausetitleImg, (400-(pausetitleImg.get_width()/2), 220))

        if gameChoice == 1:
            resumeButton = Button(resumeImg, 100, 420, Button_width,
                                  Button_height, clickresumeImg, 100, 418, resumegame)
            restartButton = Button(
                restartImg, 260, 420, Button_width, Button_height, clickrestartImg, 260, 418, main)
            saveButton = Button(saveImg, 415, 420, Button_width,
                                Button_height, clicksaveImg, 415, 418, savegame)
            quitButton = Button(quitImg, 550, 420, Button_width,
                                Button_height, clickQuitImg, 550, 418, mainmenu)

        else:
            resumeButton = Button(resumeImg, 100, 420, Button_width,
                                  Button_height, clickresumeImg, 100, 418, resumegame)
            restartButton = Button(restartImg, 330, 420, Button_width,
                                   Button_height, clickrestartImg, 330, 418, dualgame)
            quitButton = Button(quitImg, 550, 420, Button_width,
                                Button_height, clickQuitImg, 550, 418, mainmenu)
...
def mainmenu():
    ...
    while menu:
        ...
        titletext = gameDisplay.blit(
            titleImg, (400-(titleImg.get_width()/2), 220))

        Button_width = startImg.get_width()
        Button_height = startImg.get_height()

        singleButton = Button(startImg, 125, 420, Button_width,
                              Button_height, clickStartImg, 125, 418, main)
        dualButton = Button(dualImg, 325, 420, Button_width,
                            Button_height, clickDualImg, 325, 418, dualgame)
        autoButton = Button(autoImg, 525, 420, Button_width,
                            Button_height, clickAutoImg, 525, 418, autogame)

        rankingButton = Button(rankingImg, 125, 520, Button_width,
                               Button_height, clickrankingImg, 125, 518, showrank)

        if(is_it_save()):
            loadButton = Button(loadImg, 325, 520, Button_width,
                                Button_height, clickloadImg, 325, 518, loadgame)
        else:
            loadButton = Button(clickloadImg, 325, 520, Button_width,
                                Button_height, clickloadImg, 325, 518, None)

        quitButton = Button(quitImg, 525, 520, Button_width,
                            Button_height, clickQuitImg, 525, 518, quitgame)
...
class Food(object):
    ...
    def randomize_position(self):
        self.position = (random.randint(0, GRID_NUM-1) * GRID_SIZE,
                         random.randint(0, GRID_NUM-1) * GRID_SIZE)
...
class Snake(object):
            ...
            if i == 0:
                if len(self.positions) == 1:
                    snake_head_image = pygame.image.load(
                        'snake_game/imgs/snake_head.png')
                else:
                    snake_head_image = pygame.image.load(
                        'snake_game/imgs/snake_head1.png')
                snake_head_image = pygame.transform.scale(
                    snake_head_image, (GRID_SIZE, GRID_SIZE))
                ...
                snake_head_image = pygame.transform.rotate(
                    snake_head_image, rotate)
                surface.blit(snake_head_image,
                             (self.positions[i][0], self.positions[i][1]))
            elif i == len(self.positions) - 1:
                snake_tail_image = pygame.image.load(
                    'snake_game/imgs/snake_tail.png')
                snake_tail_image = pygame.transform.scale(
                    snake_tail_image, (GRID_SIZE, GRID_SIZE))
                if self.directions[i] == UP:
                    rotate = 180
                elif self.directions[i] == DOWN:
                    rotate = 0
                    ...
                    rotate = 90
                snake_tail_image = pygame.transform.rotate(
                    snake_tail_image, rotate)
                surface.blit(snake_tail_image,
                             (self.positions[i][0], self.positions[i][1]))
            else:
                r = pygame.Rect(
                    (self.positions[i][0], self.positions[i][1]), (GRID_SIZE, GRID_SIZE))
    ...
    def move(self, screen):
        ...
        new = (((now[0] + (x*GRID_SIZE)) % SCREEN_SIZE),
               (now[1] + (y*GRID_SIZE)) % SCREEN_SIZE)
